...
impl Database {
    ...
    /// 获取某年某月下每天练了几个动作
    pub async fn get_daily_exercise_count(
        &self,
        year: i32,
        month: u32,
    ) -> Result<Vec<DailyExerciseCount>, sqlx::Error> {
        sqlx::query_as::<_, DailyExerciseCount>(
            "SELECT
            CAST(strftime('%d', created_at) AS INTEGER) as day,
            CAST(COUNT(DISTINCT exercise_id) AS INTEGER) as count
         FROM records
...
mod tests {
    ...
    #[tokio::test]
    async fn test_get_daily_exercise_count() {
        let dir = tempdir().expect("创建临时目录失败");
        let path = dir.path().to_str().expect("路径转换失败");
        let db = Database::new(path).await.expect("数据库初始化失败");
        ...
        let e1_id = db
            ...
            .unwrap();
        let e2_id = db
            .add_exercise(r_id, "上斜卧推", 4, "10", "", "kg")
            .await
            .unwrap();

        // 2. 添加记录（同一天的同一动作多条记录，只应算 1 个动作）
        let rec1 = db.add_record(e1_id, 40.0, Some(10)).await.unwrap();
        let rec2 = db.add_record(e1_id, 40.0, Some(10)).await.unwrap();
        let rec3 = db.add_record(e1_id, 45.0, Some(8)).await.unwrap();
        let rec4 = db.add_record(e2_id, 50.0, Some(5)).await.unwrap();
        let rec5 = db.add_record(e1_id, 50.0, Some(5)).await.unwrap();
        let rec6_out_of_bounds = db.add_record(e2_id, 50.0, Some(5)).await.unwrap();
        ...
        sqlx::query(update_time_query)
            ...
            .unwrap();
        sqlx::query(update_time_query)
            .bind("2026-04-12 18:30:00")
            .bind(rec4)
            .execute(&db.pool)
            .await
            .unwrap();

        // -> 归属到 2026年4月28日（1 个动作）
        sqlx::query(update_time_query)
        ...
        sqlx::query(update_time_query)
            ...
            .bind(rec6_out_of_bounds)
            .execute(&db.pool)
            .await
            .unwrap();

        // 4. 执行测试：查询 2026 年 4 月的数据
        let daily_counts = db.get_daily_exercise_count(2026, 4).await.unwrap();

        // 5. 验证结果
        assert_eq!(daily_counts.len(), 3, "2026年4月应该只有3天包含训练记录");

        assert_eq!(daily_counts[0].day, 5);
        assert_eq!(daily_counts[0].count, 1);

        assert_eq!(daily_counts[1].day, 12);
        assert_eq!(daily_counts[1].count, 2);
